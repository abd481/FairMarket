...
def canonicalize_location(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['location'] = df['location_prefix'].map(freq_table).fillna(df['location_canonical'])
...
def drop_columns(df: pd.DataFrame) -> pd.DataFrame:
    df = df.drop(columns=[
        'checksum',
        'reactivated_date', 'transformed_at'
    ])
    return df
...
def filter_price_anomalies(df: pd.DataFrame, pps_threshold: float = 0.30, min_district_size: int = 3) -> pd.DataFrame:
    """
