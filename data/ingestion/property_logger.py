...
class PropertyLogger:
    ...
    def log_rejection(self, prop, failed_rules: List[str]) -> Optional[str]:
        ...
        try:
            ...
            if self.rejected_collection is not None:
                ...
                logger.warning(
                    "MongoDB connection unavailable - rejection not saved to database"
                )
                return None
    ...
    def log_approved(self, prop, checksum: str) -> Optional[str]:
        ...
        try:
            ...
            if self.raw_collection is not None:
                ...
                logger.debug(
                    f"Saved approved property to raw_listings: {result.inserted_id}"
                )
                ...
                logger.warning(
                    "MongoDB connection unavailable - approved property not saved to database"
                )
                return None
    ...
    def log_scraping_end(self, source: str, total_scraped: int) -> None:
        ...
        logger.info(
            f"✅ Scraping completed | Source: {source} | Total Properties: {total_scraped}"
        )

    def log_scraping_error(self, source: str, link: str, error: str) -> None:
        """Log errors during scraping."""
        logger.error(
            f"❌ Scraping error | Source: {source} | Link: {link} | Error: {error}"
        )
