...
# Some modules (e.g. models.predict) create an SQLAlchemy engine at import time.
# Use a placeholder URI so importing them in CI (no .env / no Postgres) is safe.
os.environ.setdefault("POSTGRES", "postgresql://test:test@localhost:5432/test")
    ...
    return pd.DataFrame(
        {
            ...
            "furnishing": np.random.choice(
                ["Furnished", "Unfurnished", "Semi-Furnished"], n
            ),
            "location": [
                (
                    "Cairo,New Cairo,5th Settlement"
                    if i % 3 == 0
                    else "Giza,6th October" if i % 3 == 1 else "Alexandria"
                )
                for i in range(n)
            ],
            ...
            "title": [
                "Finished apartment " + str(i) + " with balcony" for i in range(n)
            ],
            "source": ["bayut"] * n,
            "price_per_sqm": [0.0] * n,
            ...
            "district": [""] * n,
            "city": [""] * n,
            "scraped_at": pd.date_range("2024-01-01", periods=n, freq="D"),
        }
    )
