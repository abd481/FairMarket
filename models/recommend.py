...
def build_index(mode): 
    ...
    # Feature eingineering (same as preprocess() and prepare_row ())
    df['compound'] = df['location'].apply(lambda x: x.split(',')[0].strip() if pd.notna(x) and ',' in x 
    ...
    # Load pipline and compute district_avg_pps 
    pipeline = joblib.load(PROJECT_ROOT / 'artifacts' / 'pipelines' / f"{mode}_pipeline.joblib" )
    ...
    metadata_cols = ['id', 'price', 'area', 'beds', 'baths', 'property_type',
                     'location', 'city', 'district', 'compound']
...
def recommend(property_id , mode, zone_filter = True , price_tolerance=0.3 , k=10) : 
    ...
    distances, indices = knn.kneighbors(query_vector.reshape(1, -1),n_neighbors=min(k * 3 , len(feature_matrix)))
    ...
    # Lookup metadata from cached DataFrame
    ...
    print(f"-"*110)
    print(f"  {'#':<3} {'Price':<12} {'Area':<5} {'Beds':<5} {'Baths':<5} "
...
def explore(city=None, district=None, property_type=None,
            price_min=None, price_max=None, k=20):
    ...
    print('-' * 120)
    print(f"  {'#':<3} {'Price':<12} {'Predicted':<12} {'Area':<5} {'Beds':<5} "
    ...
    for i, (_, row) in enumerate(df.iterrows(), 1):
        ...
        pred = f"EGP {row['predicted_price']:,.0f}" if pd.notna(row.get('predicted_price')) else 'N/A'
        print(f"  {i:<3} EGP {row['price']:>8,.0f} {pred:<12} {row['area']:<4.0f} "
...
if __name__ == "__main__" : 
    ...
    parser.add_argument('--mode',choices=['only_villas','no_villas'],default='no_villas')
    ...
    if args.explore: 
        explore(city=args.city,district=args.district,property_type=args.property_type
                ,price_min=args.price_min,price_max=args.price_max,k=args.k)
    if args.build_index:
        build_index(args.build_index)
    elif args.recommend:
        recommend(args.recommend,args.mode,price_tolerance=args.price_tolerance,k=args.k)
