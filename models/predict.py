...
def load_artifacts(mode):
    """Load the pipeline, model, district price-per-sqm lookup, and calibration map for a mode."""
...
def classify_mode(property_type):
    """Route a property type to its model mode (villa vs. non-villa)."""
    return "only_villas" if property_type in VILLA_TYPES else "no_villas"


def prepare_row(row, district_pps, global_pps):
    """Apply the same feature engineering used at training time to a single row."""
    row = row.copy()
...
def predict_single(property_id):
    ...
    # Look up the calibrated residual band for this prediction's bin
...
def predict_batch():
    """Predict prices for all listings that don't already have a prediction, and save them."""
    try:
        df = pd.read_sql(
            """
    ...
    for group_df, mode in [(villa_df, "only_villas"), (no_villa_df, "no_villas")]:
        ...
        pipeline, model, district_pps, global_pps, calib = load_artifacts(mode)
        prepared = group_df.apply(
            lambda r: prepare_row(r, district_pps, global_pps), axis=1
        )
...
def predict_from_features(
    ...
    """Predict a price from a raw feature dict (e.g. an API request) rather than a stored row."""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--property-id", type=int, help="Predict a single property by ID"
    )
